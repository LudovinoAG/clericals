...
<html>
<head>
	<title>SV - Registros General</title>
	<meta charset='utf-8' />
  ...
  <table class="table text-center table-hover">
		...
		<?php
			if($conexion = mysqli_connect('127.0.0.1','root','123456')){
					...
					$OrdenEdit = $_GET['OS'];
					$sql = "SELECT * FROM registromac WHERE Status='Abierto' AND RegOS='$OrdenEdit'";
					$result = mysqli_query($conexion, $sql) or die ('No ejecutado');
					...
					while($RegAgent=mysqli_fetch_array($result))
						{
							...
							echo "<tr id='filasData'>
									<td><a href='updatemac2.php?ID=$RegAgent[0]'><button class='btn btn-info btn-lg'><span class='glyphicon glyphicon-pencil'></span></button></a></td>
									<td>$RegAgent[3]</td>
									<td>$RegAgent[4]</td>
									<td>$RegAgent[6]</td>
